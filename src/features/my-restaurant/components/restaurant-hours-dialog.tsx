...
function DayRow({ day, onChange }: DayRowProps) {
  const setPeriod = (index: number, patch: Partial<{ open: string; close: string }>) => {
    const periods = day.periods.map((p, i) => (i === index ? { ...p, ...patch } : p))
  ...
  return (
    <div className='space-y-2 border-b py-3 last:border-b-0'>
      <div className='flex items-center justify-between'>
        <span className='w-24 shrink-0 font-medium'>{dayLabel(day.day_of_week)}</span>
        <div className='flex items-center gap-4 text-sm'>
          <label className='flex items-center gap-2'>
            <Switch
              checked={day.closed}
              onCheckedChange={(closed) =>
                onChange({ ...day, closed, open_24h: closed ? false : day.open_24h })
              }
            />
            Closed
          </label>
      ...
      {!day.closed && !day.open_24h && (
        <div className='space-y-2'>
            ...
            <div key={index} className='flex items-center gap-2'>
              ...
              {day.periods.length > 1 && (
                <Button
                  ...
                  variant='ghost'
                  size='icon'
                  onClick={() => onChange({ ...day, periods: day.periods.filter((_, i) => i !== index) })}
                >
                  <X />
                </Button>
              )}
          ...
          <Button
            ...
            variant='outline'
            size='sm'
            onClick={() => onChange({ ...day, periods: [...day.periods, { ...defaultPeriod }] })}
          >
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.setQueryData(['my-restaurant'], (current: { restaurant: Restaurant } | undefined) =>
        current ? { restaurant: { ...current.restaurant, hours: data.hours } } : current
      )
      onOpenChange(false)
    },
    ...
    >
      <DialogContent className='sm:max-w-lg'>
        ...
        <div className='h-105 w-[calc(100%+0.75rem)] overflow-y-auto pe-3'>
          {days.map((day, index) => (
            <DayRow key={day.day_of_week satisfies DayOfWeek} day={day} onChange={(d) => setDay(index, d)} />
          ))}
        </div>
        <DialogFooter>
